...
if (isset($_POST['submit'])) {
    // They take values using the name attribute
    ...
    if (!empty($_FILES['resume']['name'])) {
        $base = $_FILES['resume']['name'];
        $resumeFileType = pathinfo($base, PATHINFO_EXTENSION);
    }

    //Setting a random non-repeatable file name. Uniqid will create a unique name based on current timestamp. We are using this because no two files can be of the same name as it will overwrite.
    ...
    if (!empty($_FILES['resume']['tmp_name'])) {
        //We move the uploaded file to the desired location
        move_uploaded_file($_FILES["resume"]["tmp_name"], $filename);
    } else {
        //If no file is uploaded, set $file to an empty string
        $file = '';
    }
